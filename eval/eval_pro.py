...
def ReadXml(xml_name, labels):
    ...
    for object in root.findall('object'):  # 找到root节点下的所有object节点
        ...
        scale = np.sqrt(np.abs(float(x2) - float(x1)) * np.abs(float(y2) - float(y1)))
...
def ReadResult(resultFile,labels):
    ...
    with io.open(resultFile,encoding='gbk') as f:
        for line in f.readlines():
            ...
            for i in range(0,round(float(resultData[1]))):
                dat = resultData[i*6+2 : i*6+8]
                label = dat[5]
                if label not in labels:
                    print(label)
                    continue
                scale = np.sqrt(np.abs(float(dat[3]) - float(dat[1])) * np.abs(float(dat[4]) - float(dat[2])))
                apresult = [int(dat[1]),int(dat[2]),int(dat[3]),int(dat[4]),float(dat[0]),scale]
                # if len(apresult) != 6:
                #     print("ap:",float(dat[0]),scale)
                resultDict[dat[5]].append(apresult)
...
def DrawScaleMAP(PRmAPDict,labels,label_prefix,colors,point):
    ...
    for label_k in range(0,len(labels)):
        ...
        plt.text(xTicks, PRmAPDict[dictKey][0], '%.4f'%PRmAPDict[dictKey][0], ha='left', va='bottom', fontsize=15)
...
labels = ['person']
annaFile = r'ImageSets/Main/smalltest.txt'
resultFile = eval_file + 'eval.txt'
saveDataName = eval_file + eval_filename + '.npy'
#%%生成数据
...
#%%读取数据
#saveData640 = np.load(r'eval-40000.npy').item()
#PRmAPDict640 = CalPRmAP(saveData640, '40000')

saveData = np.load(saveDataName, allow_pickle=True).item()
PRmAPDict = CalPRmAP(saveData, eval_filename)
...
#scale_lines = [ '-', '--', ':', '-.', '--', '--']
#DrawPR(PRmAPDict640,labels,'40000',label_colors,scale_lines)

label_colors = ['blue','magenta', 'lime']
scale_lines = [ '-', '--', ':', '-.', '--', '--']
DrawPR(PRmAPDict,labels,eval_filename,label_colors,scale_lines)
...
#DrawScaleMAP(PRmAPDict640,['person'],'40000',colors,'o')

colors = ['blue','magenta', 'lime']
DrawScaleMAP(PRmAPDict,['person'],'93000',colors,'*')
