...
parser.add_argument('--save_folder', default='test/10test/', type=str,
...
def test_net(save_folder, net, cuda, testset, transform, thresh):
    img_test = 10
    for index in range(img_test):
        print('Testing image {:d}/{:d}....'.format(index + 1, img_test))
            ...
            for i in range(detections.size(1)):
                ...
                while detections[0, i, j, 0] >= 0.6:
                    ...
                    if ifcut == True:
                        ...
                        coords = [int(pt[0]), int(pt[1]), int(pt[2]), int(pt[3]), score]
                    coords_list.append(coords)
                    pred_num += 1
                    j += 1
        coords_list = np.array(coords_list)
        coords_list = py_nms(coords_list, thresh, False)
        draw_rec(save_folder, img_org, img_id, coords_list)
